<div class="p-6 bg-gradient-to-br from-blue-50 to-indigo-100 rounded-lg">
    ...
    <div class="bg-white p-4 rounded-lg shadow-sm">
        <div class="flex items-center mb-3">
            ...
            <span class="font-semibold text-gray-700">{{ $title ?? 'Chứng từ thanh toán' }}</span>
        </div>

        @php
        // Chọn đường dẫn file hiển thị: ưu tiên filePath truyền vào, nếu không có thì fallback
        $currentFilePath = $filePath ?? ($payment->bill_path ?: $payment->receipt_path);
        // Tự động chọn route phù hợp nếu không truyền vào từ ngoài
        $fileUrl = $fileUrl
            ?? ($currentFilePath === $payment->bill_path
        ...
        @if(!empty($currentFilePath) && !empty($fileUrl))
        @if($isImage)
        <div class="mb-4">
